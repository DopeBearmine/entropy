...
fn main() {
    ...
    let result = entropy(data, Some("data"), Some(1.5));
    println!("Data-based Entropy: {}  |  Bin Size: 1.5", result);
    let kde: f64 = entropy(data, Some("kde"), None);
    ...
    let uniform_vals: Vec<f64> = (0..size)
        ...
        .collect();
    println!("Uniform distrobution: {}", entropy(uniform_vals.as_slice(), Some("kde"), None));
    kde_plot(uniform_vals.as_slice());
    ...
    let normal_vals: Vec<f64> = (0..size) // Generate 10 samples
        ...
        .collect();
    println!("Normal distrobution: {}", entropy(normal_vals.as_slice(), Some("kde"), None));
    kde_plot(normal_vals.as_slice());
}